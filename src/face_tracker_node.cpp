...
class Face_Detector
{
  ...
  public:
    ...
    void detectAndDraw(Mat& img, CascadeClassifier& cascade)
    {
        ...
        for (size_t i = 0; i < faces.size(); i++)
        {
            ...
            if (0.75 < aspect_ratio && aspect_ratio < 1.3)
            {
                ...
                rectangle(img, cvPoint(cvRound(r.x * scale), cvRound(r.y * scale)),
                          cvPoint(cvRound((r.x + r.width - 1) * scale), cvRound((r.y + r.height - 1) * scale)), color,
                          3, 8, 0);
        }
        ...
        putText(img, "Right", cvPoint(305, 240), FONT_HERSHEY_SIMPLEX, 1, cvScalar(255, 0, 0), 2, CV_AA);
